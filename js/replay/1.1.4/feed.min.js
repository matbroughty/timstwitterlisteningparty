...
function loadTweet(a){var b=document.getElementById("tweet-feed-"+a);null!==b&&(b=$(b).attr("data-url"),null!==b&&void 0!==b&&$.ajax({type:"GET",url:"https://publish.twitter.com/oembed?url="+b+"&omit_script=true&dnt=true&theme="+(1===darkMode?"dark":"light"),dataType:"jsonp",success:function(b){$("#tweet-feed-"+a).html(b.html);4>a&&(loadTweet(a+1),0===a&&(timeout=setTimeout("showIt()",250)));twttr.widgets.load(document.getElementById("#tweet-feed-"+a))},error:function(b,c,d){if(404!==b.status)alert("Unable to load feed - please check that you are able to access Twitter on this device. If the problem persists, please try reloading the page. ["+
b.status+" - "+c+"]");else{console.log("404 - tweet deleted");b=document.getElementById("tweet-feed-"+a);c=b.style.display;b.style="border-radius:15px;border:1px solid #ccc;padding:5px;width:100%;max-width:550px;margin-top:5px;";b.style.display=c;c="@"+b.innerHTML.trim();d=b.getAttribute("data-datetime");var f=b.getAttribute("data-text");if(null===f||3>f.length)f="This tweet has been deleted";b.innerHTML='<table style="width:98%;max-width:550px;"><tr><td style="text-align:left;width:1%;padding-right:7px;vertical-align:top;"><img src="../../img/twitter.png" alt="CD" width="45px"></td><td style="text-align:left;width:100%;"><b>'+
c+'</b><div class="main-colour" style="line-height:1.1em;">'+f+'</div><div style="font-size:0.8em;padding-top:2px;line-height:1.1em;color:#777777;">'+d+"</div></td></tr></table>";4>a&&(loadTweet(a+1),0===a&&(timeout=setTimeout("showIt()",250)))}}}))}
...
counter++,a=document.getElementById("tweet-feed-"+counter),null!==a)b=Number($(a).attr("data-delay")),loadTweet(counter+4);else{var e=document.getElementById("tweet-feed-"+(counter-1));$(e).slideDown("slow");e=document.getElementById("timer-div");$(e).slideUp("slow");ga("send",{hitType:"event",eventCategory:"feed",eventAction:"ended",eventLabel:urlpage})}if(null!==a){mssofar=(new Date).getTime()-startms;a=delaysofar+b-mssofar;0>a&&(a=1);timeOfNextTweet=(new Date).getTime()+a;timeout=setTimeout("showIt()",
a);firstTweetLoaded?(document.getElementById("timer").classList.remove("blinking"),document.getElementById("timer").classList.remove("blinking-dark"),document.getElementById("timer").classList.add("not-blinking")):(firstTweetLoaded=!0,pauseText="Press here to continue when you have started the music/video ",document.getElementById("stand-by-arrow").style.display="block",document.getElementById("darkMode").style.display="block",playPause());var c=setInterval(function(){if(paused)document.getElementById("timer").innerHTML=
pauseText;else{var a=(new Date).getTime(),b=Math.floor((timeOfNextTweet-a)/1E3);1>b?(clearInterval(c),document.getElementById("timer").innerHTML="Next tweet coming up..."):document.getElementById("timer").innerHTML="Next tweet in "+b+(1!==b?" seconds":" second");updateNowPlaying(a-startms)}},1E3)}}}
function updateNowPlaying(a){var b=document.getElementById("now-playing");if(null!==b){b.style.display="block";b=document.getElementsByClassName("track-list");var e=a/1E3;a=-1;for(var c=1;c<b.length;c++)if(b[c].getAttribute("data-start")>e){a=c-1;break}-1===a&&(a=b.length-1);c=b[a];var d=c.getAttribute("data-start"),f=document.getElementById("track-time"),g=document.getElementById("track-title");d=e-d;e=Math.floor(d/60);d="0"+Math.floor(d%60);g.innerHTML=c.getAttribute("data-name");f.innerHTML=a<
b.length-1?e+":"+d.substr(d.length-2):""}}
